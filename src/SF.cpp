...
RIFF::Chunk *GetMandatoryChunk(RIFF::List *list, uint32_t chunkId)
{
    ...
    if (ck == NULL)
        throw Exception("Mandatory chunk in RIFF list chunk not found: " + ToString(chunkId));
    return ck;
}
...
int Sample::GetChannelCount() { return ChannelCount; }

long Sample::GetTotalFrameCount() { return (End - Start); }

/**
 * @returns The frame size in bytes
 */
int Sample::GetFrameSize() { return ChannelCount * ((pCkSm24 != NULL) ? 3 : 2); }

bool Sample::HasLoops() { return StartLoop != 0 && EndLoop != 0; }
 ...
 */
unsigned long Sample::ReadAndLoop(void *pBuffer, unsigned long FrameCount,
                                  PlaybackState *pPlaybackState, Region *pRegion)
{
    ...
    if (pRegion->HasLoop)
    {
        do
        {
            ...
            readsamples = Read(&pDst[totalreadsamples * GetFrameSize()],
...
Region::Region()
{
    ...
    startloopAddrsOffset = startloopAddrsCoarseOffset = endloopAddrsOffset =
...
double Region::GetEG1PreAttackDelay(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG1PreAttackDelay == NONE)
                  ? EG1PreAttackDelay
                  : pPresetRegion->EG1PreAttackDelay + EG1PreAttackDelay;
...
double Region::GetEG1Attack(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG1Attack == NONE)
                  ? EG1Attack
                  : pPresetRegion->EG1Attack + EG1Attack;
...
double Region::GetEG1Decay(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG1Decay == NONE)
                  ? EG1Decay
                  : pPresetRegion->EG1Decay + EG1Decay;
...
int Region::GetEG1Sustain(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG1Sustain == NONE)
                  ? EG1Sustain
                  : pPresetRegion->EG1Sustain + EG1Sustain;
...
double Region::GetEG1Release(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG1Release == NONE)
                  ? EG1Release
                  : pPresetRegion->EG1Release + EG1Release;
...
double Region::GetEG2PreAttackDelay(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG2PreAttackDelay == NONE)
                  ? EG2PreAttackDelay
                  : pPresetRegion->EG2PreAttackDelay + EG2PreAttackDelay;
...
double Region::GetEG2Attack(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG2Attack == NONE)
                  ? EG2Attack
                  : pPresetRegion->EG2Attack + EG2Attack;
...
double Region::GetEG2Decay(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG2Decay == NONE)
                  ? EG2Decay
                  : pPresetRegion->EG2Decay + EG2Decay;
...
int Region::GetEG2Sustain(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG2Sustain == NONE)
                  ? EG2Sustain
                  : pPresetRegion->EG2Sustain + EG2Sustain;
...
double Region::GetEG2Release(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->EG2Release == NONE)
                  ? EG2Release
                  : pPresetRegion->EG2Release + EG2Release;
...
int Region::GetModEnvToPitch(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->modEnvToPitch == NONE)
                  ? modEnvToPitch
                  : pPresetRegion->modEnvToPitch + modEnvToPitch;
...
int Region::GetModLfoToPitch(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->modLfoToPitch == NONE)
                  ? modLfoToPitch
                  : pPresetRegion->modLfoToPitch + modLfoToPitch;
...
int Region::GetModEnvToFilterFc(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->modEnvToFilterFc == NONE)
                  ? modEnvToFilterFc
                  : pPresetRegion->modEnvToFilterFc + modEnvToFilterFc;
...
int Region::GetModLfoToFilterFc(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->modLfoToFilterFc == NONE)
                  ? modLfoToFilterFc
                  : pPresetRegion->modLfoToFilterFc + modLfoToFilterFc;
...
double Region::GetModLfoToVolume(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->modLfoToVolume == NONE)
                  ? modLfoToVolume
                  : pPresetRegion->modLfoToVolume + modLfoToVolume;
...
double Region::GetFreqModLfo(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->freqModLfo == NONE)
                  ? freqModLfo
                  : pPresetRegion->freqModLfo + freqModLfo;
...
double Region::GetDelayModLfo(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->delayModLfo == NONE)
                  ? delayModLfo
                  : pPresetRegion->delayModLfo + delayModLfo;
...
int Region::GetVibLfoToPitch(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->vibLfoToPitch == NONE)
                  ? vibLfoToPitch
                  : pPresetRegion->vibLfoToPitch + vibLfoToPitch;
...
double Region::GetFreqVibLfo(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->freqVibLfo == NONE)
                  ? freqVibLfo
                  : pPresetRegion->freqVibLfo + freqVibLfo;
...
int Region::GetInitialFilterQ(Region *pPresetRegion)
{
    int val = (pPresetRegion == NULL || pPresetRegion->initialFilterQ == NONE)
                  ? initialFilterQ
                  : pPresetRegion->initialFilterQ + initialFilterQ;
...
int InstrumentBase::GetRegionCount() { return (int)regions.size(); }
...
Query::Query(InstrumentBase &instrument) : instrument(instrument) { i = 0; }
...
Region *Preset::CreateRegion()
{
    ...
    r->EG1PreAttackDelay = r->EG1Attack = r->EG1Hold = r->EG1Decay = r->EG1Sustain = r->EG1Release =
        NONE;
    r->EG2PreAttackDelay = r->EG2Attack = r->EG2Hold = r->EG2Decay = r->EG2Sustain = r->EG2Release =
        NONE;
...
{
    if (!pRIFF)
        throw Exception("NULL pointer reference to RIFF::File object.");
    this->pRIFF = pRIFF;
...
RIFF::File *File::GetRiffFile() { return pRIFF; }
...
{
    return LoadSampleDataWithNullSamplesExtension(GetTotalFrameCount(),
 ...
 * samples by dividing it by the frame size of the sample:
 * @code
 *  buffer_t buf       = pSample->LoadSampleDataWithNullSamplesExtension(acquired_samples,
 * null_samples); long cachedsamples = buf.Size / pSample->FrameSize;
 * @endcode
 * The method will add \a NullSamplesCount silence samples past the
 * official buffer end (this won't affect the 'Size' member of the
 * buffer_t structure, that means 'Size' always reflects the size of the
 ...
 * @see                      ReleaseSampleData(), Read(), SetPos()
 */
Sample::buffer_t Sample::LoadSampleDataWithNullSamplesExtension(unsigned long SampleCount,
                                                                uint NullSamplesCount)
{
    if (SampleCount > GetTotalFrameCount())
        SampleCount = GetTotalFrameCount();
    if (RAMCache.pStart)
...
unsigned long Sample::SetPos(unsigned long SampleCount)
{
    ...
    if (pCkSm24)
        pCkSm24->SetPos(Start + SampleCount, RIFF::stream_start);
    return SampleCount;
}
 ...
 */
unsigned long Sample::GetPos() { return (pCkSmpl->GetPos() - (Start * 2)) / 2; }

// Actual implementation of Sample::Read*() code. Wrapped into a template for a) runtime effeciency
// and b) code redundancy reasons.
template <bool CLEAR>
inline unsigned long ReadSample(Sample *pSample, void *pBuffer, unsigned long SampleCount,
                                Sample::buffer_t *tempBuffer = NULL)
{
    ...
    if (pSample->GetFrameSize() / pSample->GetChannelCount() == 3 /* 24 bit */)
    {
        ...
        if (pSample->SampleType == Sample::MONO_SAMPLE ||
            pSample->SampleType == Sample::ROM_MONO_SAMPLE)
        {
        ...
        else if (pSample->SampleType == Sample::LEFT_SAMPLE ||
                 pSample->SampleType == Sample::ROM_LEFT_SAMPLE)
        {
            pSample->pCkSmpl->Read(pTmpBuf, SampleCount, 2);
            pSample->pCkSm24->Read(pTmpBuf + SampleCount * 2, SampleCount, 1);
            for (long i = SampleCount - 1; i >= 0; i--)
        ...
        else if (pSample->SampleType == Sample::RIGHT_SAMPLE ||
                 pSample->SampleType == Sample::ROM_RIGHT_SAMPLE)
        {
            pSample->pCkSmpl->Read(pTmpBuf, SampleCount, 2);
            pSample->pCkSm24->Read(pTmpBuf + SampleCount * 2, SampleCount, 1);
            for (long i = SampleCount - 1; i >= 0; i--)
    ...
    else
    {
        ...
        if (pSample->SampleType == Sample::LEFT_SAMPLE ||
            pSample->SampleType == Sample::ROM_LEFT_SAMPLE)
        {
            pSample->pCkSmpl->Read(pTmpBuf, SampleCount, 2);
            for (long i = SampleCount - 1; i >= 0; i--)
    ...
    if (pSample->pCkSmpl->GetPos() > (pSample->End * 2))
    {
        ...
        std::cerr << "Total number of frames: " << pSample->GetTotalFrameCount() << std::endl
                  << std::endl;
    }
    return SampleCount;
}
 ...
 */
String libraryName() { return PACKAGE; }

/**
 * Returns version of this C++ library.
 */
String libraryVersion() { return VERSION; }
